...
def fetch_battle_log_first_pass(player_tag, brawler_stats, seen_players):
...
def fetch_battle_log_sequential_passes(player_tag, brawler_stats):
    BASE_URL = f'https://api.brawlstars.com/v1/players/{player_tag.replace("#", "%23")}/battlelog'
    response = requests.get(BASE_URL, headers=HEADERS)
    if response.status_code == 200:
        battle_log = response.json()
...
def main():
    seen_players = set()
    brawler_stats = {}
    fetch_battle_log_first_pass(PLAYER_TAG, brawler_stats, seen_players)
    ...
    for new_player_tag in seen_players:
        ...
        if new_player_tag != PLAYER_TAG:
            fetch_battle_log_sequential_passes(new_player_tag, brawler_stats)
